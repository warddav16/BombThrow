...
DeferredBuffers::DeferredBuffers(ID3D11Device* device, int textureWidth, int textureHeight, float screenDepth, float screenNear)
{
	...
	for(i=0; i<BUFFER_COUNT; i++)
	{
		...
		result = device->CreateShaderResourceView(m_renderTargetTextureArray[i], &shaderResourceViewDesc, &resourceView);
		if(!FAILED(result))
		{
			m_textures[i]->SetTexture(resourceView);
		}
	}
...
DeferredBuffers::~DeferredBuffers(void)
{
